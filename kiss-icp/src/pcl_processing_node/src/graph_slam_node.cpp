...
private:
    ...
    void odometryCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
        ...
        double distance = (current_pose.translation() - last_odometry_in_queue_.translation()).norm();
        double current_yaw = getYawFromQuaternion(Eigen::Quaterniond(current_pose.rotation()));
        double last_yaw = getYawFromQuaternion(Eigen::Quaterniond(last_odometry_in_queue_.rotation()));
        double yaw_difference = std::abs(current_yaw - last_yaw);
